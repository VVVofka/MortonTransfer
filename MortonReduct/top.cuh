...
static __device__ __host__ __forceinline__ uint32_t get_A8(const uint32_t src){ // src: 32 active bit; return 2*4 bit
	constexpr uint64_t M = 0x1111'1111'1111'1111ULL;
	uint32_t sum = (src & M) + ((src >> 1) & M) + ((src >> 2) & M) + ((src >> 3) & M);
	sum >>= 1;			// 1 if 2 or 3
	sum |= sum >> 1;  // or if 4 ( res in pos 0)
...
static __global__ void glDnMid3(const __half2* __restrict__ f_up_in,
								...
								__half2* __restrict__ f_out){
	__shared__ uint64_t src_a[32], a8[8], a7[2];
	...
	if(threadIdx.x < 32)
		src_a[threadIdx.x] = in_val[32 * blockIdx.x + threadIdx.x];
	__syncwarp();

	if(threadIdx.x < 8)
		a8[threadIdx.x] = reduct64by1bit(src_a + threadIdx.x * 4);
	__syncwarp();

	if(threadIdx.x < 2)
		a7[threadIdx.x & 1] = reduct64by1bit(a8 + (threadIdx.x & 1) * 4);
	__syncwarp();
