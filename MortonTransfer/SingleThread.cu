...
#pragma unroll
	for(unsigned i = 0; i < 32; ++i){
		...
		if(out_index >= total_values) break;
		...
		unsigned x_in = (x - shift.x + size_side) % size_side;
		unsigned y_in = (y - shift.y + size_side) % size_side;

		unsigned in_index = EncodeMorton2(x_in, y_in);
		unsigned val = get2bits64(data_in, in_index);

		result |= ((uint64_t)val & 0x3ULL) << (i * 2);
	}
	data_out[tid] = result;
}
// ---------------------------
...
void test1(){
	...
	set2bits64(h_input.data(), 25, 3);	//5, 2
...
void test2(){
	...
	const unsigned N = 12; // 2^5 = 32 x 32 2^12=4096
	const unsigned size_side = 1u << N;
	const unsigned total_values = size_side * size_side;
	const unsigned total_words = (total_values + 31) / 32;
